...
class Assistant:
    ...
    def _prepare_prompts(self):
        ...
        **Твоя задача:**
        ...
        1.  **Заполняй все пустые поля:** Твоя главная задача — заполнить ВСЕ пустые строковые поля (`contract_name`, `creature_name` и т.д.) и пустой массив `abilities`.
        2.  **Не изменяй существующее:** Если поле в контракте уже заполнено, не меняй его.
        3.  **Генерируй НЕСКОЛЬКО способностей:** Если массив `abilities` пуст, ты **обязан** сгенерировать как минимум 2-3 сбалансированные способности, которые соответствуют запросу и тематике.
        4.  **Анализируй контекст:** Внимательно изучи анкету персонажа (ранг, архетипы, фракция) и запрос игрока, чтобы твои идеи были логичными и уместными.
        5.  **Следуй структуре:** Твой ответ должен быть строго в формате JSON, соответствующем предоставленной схеме.
