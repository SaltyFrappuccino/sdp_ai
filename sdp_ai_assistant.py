...
class Assistant:
    ...
    def _prepare_prompts(self):
        """
        Готовит и компилирует шаблоны промптов для разных задач из config.py.
        """
        self.creative_idea_prompt = ChatPromptTemplate.from_template(config.CREATIVE_IDEA_PROMPT)
        self.json_formatter_prompt = ChatPromptTemplate.from_template(config.JSON_FORMATTER_PROMPT)
        self.validation_prompt = ChatPromptTemplate.from_template(config.VALIDATION_PROMPT)
        self.idea_prompt = ChatPromptTemplate.from_template(config.IDEA_GENERATION_PROMPT)
